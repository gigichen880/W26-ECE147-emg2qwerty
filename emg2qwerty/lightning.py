...
class CnnRnnCTCModule(pl.LightningModule):
    ...
    def _step(
        ...
        # Exact temporal lengths after the encoder's three stride-2 Conv1d blocks.
        emission_lengths = self.encoder.output_lengths(input_lengths)
...
class CnnCTCModule(pl.LightningModule):
    ...
    def __init__(
        ...
        self.model = nn.Sequential(
            ...
            nn.Flatten(start_dim=2),
            self.encoder,
            nn.Linear(self.encoder.output_dim, charset().num_classes),
            nn.LogSoftmax(dim=-1),
        )
...
# We implemented ConvTransformerCTCModule, but not included it in the report as it does not improve the performance.
